...
from nnunetv2.training.loss.kspace import CombinedImageKSpaceLoss
...
class nnUNetTrainerMRCT_AblationBase(nnUNetTrainer):
	def __init__(
		self,
		plans: dict,
		configuration: str,
	...
	def on_epoch_end(self):
		...
		if self._best_ema is None or self.logger.my_fantastic_logging[best_metric][-1] < self._best_ema:
			...
			self.print_to_log_file(f"Yayy! New best loss: {np.round(self._best_ema, decimals=4)}")
...
class nnUNetTrainerMRCT_Ablation_L1_250(nnUNetTrainerMRCT_AblationBase):
	def _build_loss(self):
		return myMAE()

class nnUNetTrainerMRCT_Ablation_L1_KSpace_Uniform_250(nnUNetTrainerMRCT_AblationBase):
	def _build_loss(self):
		return CombinedImageKSpaceLoss(
			image_weight=1.0,
			kspace_weight=0.15,
			gradient_weight=0.0,
			kspace_config={"band_weights": [1.0, 1.0, 1.0]},
			image_loss="l1",
		)


class nnUNetTrainerMRCT_Ablation_L1_KSpace_Weighted_250(nnUNetTrainerMRCT_AblationBase):
	def _build_loss(self):
		return CombinedImageKSpaceLoss(
			image_weight=1.0,
			kspace_weight=0.15,
			gradient_weight=0.0,
			kspace_config={"band_weights": [1.5, 1.0, 2.0]},
			image_loss="l1",
		)


class nnUNetTrainerMRCT_Ablation_L1_Gradient_250(nnUNetTrainerMRCT_AblationBase):
	def _build_loss(self):
		return CombinedImageKSpaceLoss(
			image_weight=1.0,
			kspace_weight=0.0,
...
class nnUNetTrainerMRCT_Ablation_L1_KSpace_Weighted_Gradient_250(nnUNetTrainerMRCT_AblationBase):
	def _build_loss(self):
		return CombinedImageKSpaceLoss(
			image_weight=1.0,
			kspace_weight=0.15,
...
class nnUNetTrainerMRCT_Ablation_L1_KSpace_Uniform_Gradient_250(nnUNetTrainerMRCT_AblationBase):
	def _build_loss(self):
		return CombinedImageKSpaceLoss(
			image_weight=1.0,
			kspace_weight=0.15,
