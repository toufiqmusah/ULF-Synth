...
def noisy_kspace(K, I_ref, signal_target):
    ...
    noise_level = np.zeros_like(mask, dtype=float)
    noise_level[mask] = 1.0
    sigma = np.sqrt(signal_power / (signal_target + 1e-8))
    noise_img = sigma * noise_level * (
...
def sample_params():
    return {
        "T2":               np.random.uniform(0.070, 0.090),
        "TE":               np.random.uniform(0.100, 0.130),
        "b0_strength":      np.random.uniform(0.020, 0.040),
        "smoothness":       np.random.uniform(30, 45),
        "k":                np.random.uniform(10, 15),
        "signal_target":    np.random.uniform(15, 50),
        "crop_ratio":       np.random.uniform(0.45, 0.55),
        "acceleration":     int(np.random.choice([2, 3], p=[0.7, 0.3])),
        "center_fraction":  np.random.uniform(0.20, 0.30),
    }
...
def process_single(hf_path, out_path, seed=None, verbose=True):
...
def process_folder(in_dir, out_dir, seed=None, verbose=True):
    ...
    for fname in files:
        ...
        file_seed = None if seed is None else seed + hash(fname) % (2**31)
        process_single(hf_path, out_path, seed=file_seed, verbose=verbose)


def main():
    parser = argparse.ArgumentParser(description="Synthesize ULF MRI from high-field NIfTI images.")
    parser.add_argument("input", help="Path to a .nii/.nii.gz file or a folder of NIfTI files.")
    parser.add_argument("output", help="Output file path (single) or output folder (folder input).")
    ...
    if os.path.isdir(args.input):
        process_folder(args.input, args.output, seed=args.seed, verbose=verbose)
    elif os.path.isfile(args.input):
        process_single(args.input, args.output, seed=args.seed, verbose=verbose)
    else:
